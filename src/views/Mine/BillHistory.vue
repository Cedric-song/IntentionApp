<template>
  <div class="history">
    ...
    <van-dialog v-model="showDialog" confirmButtonText="关闭">
      <van-field v-model="form.fullMerchantsName" label="集团名称" />
      <van-field v-model="form.tradeNo" label="订单号" />
      <van-field :value="formatTime(form.createOrderTime)" label="订单时间" />
      <van-field :value="form.phoneNo" label="电话号码" />
      <van-field :value="`${form.goodsName}元`" label="套餐" />
      <van-field :value="`${form.orderAmt}元`" label="支付金额" />
    </van-dialog>
  </div>
</template>
